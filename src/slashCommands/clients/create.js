...
module.exports = {
    data: {
        name: 'create',
        description: 'Create a bot perso',
        options: [
            {
                type: 'USER',
                name: 'user',
                description: 'The user to create the bot',
                required: true
            }
        ]
    },
    run: async (ftSecurity, interaction) => {
        ...
        const row = new MessageActionRow().addComponents(
            new MessageSelectMenu().setPlaceholder('Configure the bot').setOptions(selectMenuOptions).setCustomId(`create.${interaction.id}`)
        )
        const clientData = {
            discordId: member.id,
            maxGuilds: 5,
        ...
        const componentFilter = {
                filter: interactionCreate => interactionCreate.customId === `create.${interaction.id}` && interactionCreate.user.id === interaction.user.id,
                time: 900000
            },
            awaitMessageFilter = {
        ...
        collector.on('collect', async (interactionCreate) => {
            ...
            switch (selectedOption) {
                    ...
                    ftSecurity._fetch(`http://localhost:5006/api/client/new`,
                        {
                            ...
                            interaction.editReply({content: `Le bot pour ${member} a été créé`})
                    })
                    break
                case 'token':
                ...
                default:
                    ...
                    if (questionAnswer.content === 'cancel') return
